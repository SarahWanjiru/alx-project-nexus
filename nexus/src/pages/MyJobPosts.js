    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="w-64 bg-gradient-to-b from-teal-800 to-teal-900 text-white flex flex-col">
        <div className="p-6">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-xs text-teal-300">RECRUITER HUB</p>
            </div>
          </div>
          ...
          <nav className="space-y-2">
            ...
            <button className="w-full flex items-center gap-3 px-4 py-3 hover:bg-teal-700 rounded-lg relative">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            ...
            <button className="w-full flex items-center gap-3 px-4 py-3 hover:bg-teal-700 rounded-lg">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="mt-auto p-6">
          <button className="w-full bg-teal-600 hover:bg-teal-500 text-white py-3 rounded-lg font-semibold flex items-center justify-center gap-2">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
        ...
        <header className="bg-white border-b border-gray-200 px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              <h2 className="text-xl font-bold">JobPortal <span className="text-blue-500">Recruiter</span></h2>
              <nav className="flex items-center gap-6">
                <button onClick={() => navigate('/admin/dashboard')} className="text-gray-600 hover:text-gray-900">Dashboard</button>
                <button className="text-blue-500 font-semibold border-b-2 border-blue-500 pb-1">My Job Posts</button>
                <button className="text-gray-600 hover:text-gray-900">Applicants</button>
        ...
        <div className="p-8">
          ...
          <div className="flex items-start justify-between mb-8">
            <div>
              ...
              <p className="text-gray-600">View and manage your current job listings and recruitment progress.</p>
            </div>
              ...
              <table className="w-full">
                <thead className="bg-teal-700 text-white">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left font-semibold">DATE POSTED</th>
                    <th className="px-6 py-4 text-left font-semibold">TOTAL APPLICANTS</th>
                ...
                <tbody className="bg-white">
                  {mockJobs.map((job, index) => (
                    <tr key={job.id} className={index !== mockJobs.length - 1 ? 'border-b border-gray-200' : ''}>
                      <td className="px-6 py-4">
                        <div>
                          <p className="font-bold text-teal-700">{job.title}</p>
                          <p className="text-sm text-gray-600">{job.employment_type} • {job.location}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-gray-700">{job.created_at}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <span className="text-2xl font-bold text-blue-500">{job.applicants}</span>
                          {job.newApplicants > 0 && (
                            <span className="text-sm text-green-600 font-semibold">+{job.newApplicants} new</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <div className={`w-8 h-8 ${getStatusBadge(job.status).class} rounded-full flex items-center justify-center text-white font-bold`}>
                            {getStatusBadge(job.status).icon}
                          </div>
                          <span className={`font-bold uppercase text-sm ${
                            job.status === 'active' ? 'text-green-600' : 
                            job.status === 'paused' ? 'text-orange-600' : 'text-gray-600'
                          }`}>
                            {job.status}
                          </span>
                        </div>
          ...
          <div className="bg-gradient-to-r from-teal-600 to-blue-600 rounded-xl p-8 flex items-center justify-between text-white">
            <div>
              <h3 className="text-2xl font-bold mb-2">Want to reach more candidates?</h3>
              <p className="text-blue-100">Boost your job posts to the top of search results and attract high-quality talent faster.</p>
            </div>
